...
const { processInbox } = require('./intake');
const fs = require('fs');
...
async function runMorning(onlyKeys) {
  const log = { runner_version: VERSION, started: new Date().toISOString(), steps: [] };
  try {
    ...
    const inbox = await processInbox().catch(e=>[{error:e.message}]); if (inbox.length) log.steps.push({ new_files_detected: inbox });
