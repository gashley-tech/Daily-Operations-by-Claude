...
const { generateReportsMCP } = require('./claude-mcp');
...
const gen = process.env.USE_MCP === 'true' ? generateReportsMCP : generateReports;
const { reports, usage, toolCalls } = await gen(ctx, defs);
log.steps.push({ usage });
if (toolCalls) log.steps.push({ mcp_tool_calls: toolCalls });
